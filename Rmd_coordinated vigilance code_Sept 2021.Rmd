...
Ground squirrels do not have a sentinel system but instead synchronise vigilance 
...
obs.sessions <- scandata %>%
  group_by(year) %>%
  summarise(n_distinct(dop))

# by year
obs.scans <- scandata %>%
  group_by(year, dop) %>%
  summarise(n=n_distinct(dos))

sum.obs.scans <- obs.scans %>% 
  summarise(mean=mean(n), sd=sd(n), n=n(), se =sd/sqrt(n))
#write.csv(sum.obs.scans, "observation hours by year.csv")

# overall
obs.scans <- scandata %>%
  group_by(dop) %>%
  summarise(n_scans=n_distinct(dos))

observation.hours <- length(unique(scandata$dos)) / 6 

sum.obs.scans <- obs.scans %>% 
  summarise(mean=mean(n_scans), sd=sd(n_scans), n=n(), se =sd/sqrt(n))
#write.csv(sum.obs.scans, "observation hours.csv")
...
# plot 
percperch$extstimulus
percperch$predator <- factor(percperch$extstimulus, levels=c("not observed", "predator"), labels = c("no", "yes"))


fig_extstimulus<- ggplot(percperch, aes(x=predator, y=perched)) +
  geom_boxplot() +
  theme_classic() +
  labs(x = "predator sighted", y="proportion of perched squirrels")
fig_extstimulus
...
fig_predator <- ggplot(extstim, aes(x=bin, y=perc, fill=extstimulus)) +
  ...
  scale_fill_viridis(discrete = T, begin= 0.2, end= 0.9, option = "E", name = "predator sighted", labels = c("no", "yes")) 
fig_predator
...
- higher body condition --> body condition dependent (only enough energy to perch when in good body condition)
...
- all individuals, all adults, all adult females in an observation period
        - all scans, scans with predators, and without predators
...
fig_coord_dop <- ggplot(vigresults_dop, aes(x=groupsize, y=diff, color=individuals)) +
                 geom_point() + # shape=1
                 theme_classic() +
...
### all data
model: proportion time spent perching - proportion typ (observed vs expected) + (1|year) + (1|group)
proportion of time spent perching as dependent variable and the proportion type (observed or expected) as independent variable, with year and social group as random factors. 
...
scanprim8_groupsize_group <- scanprim8 %>% 
  ...
  dplyr::summarise(groupsize=n_distinct(id))
vigresult <- left_join(result, scanprim8_groupsize_group , by=c("year", "group"))

# compare observed vs expected at least 1 perched ----
range(vigresult$Pobs)
range(vigresult$Pexp)

## summarize
result_long <- vigresult %>% 
  #dplyr::select(-diff) %>%
...
### mixed model with group as random factor (beta distribution) -----
range(result_long1$prop)
result_long1$prop[result_long1$prop==0] <- 0.001

glimpse(result_long1)
result_long1$group<- as.factor(result_long1$group)
result_long1$year<- as.factor(result_long1$year)
result_long1$P<-factor(result_long1$type, levels = c("Pobs", "Pexp"))


mfull<- glmmadmb(prop ~ P + (1|year) + (1|group) , 
                 data = result_long1,
                 family="beta")
summary(mfull)

m0<- glmmadmb(prop ~ 1 + (1|year) + (1|group) , 
                 data = result_long1,
                 family="beta")
summary(m0)
...
## check group size effect --> not part of a priori predictions
#mfull.group<- glmmadmb(prop ~ P + groupsize + (1|year) + (1|group) , 
#                 data = result_long1,
#                 family="beta")
#summary(mfull.group)
#lrtest(mfull.group, m0)
...
bxp <- ggpaired(result_long1, x = "type", y = "prop", 
         order = c("Pobs", "Pexp"),
         ylab = "P", xlab = "")
bxp

gghistogram(result, x = "diff", y = "..density..", 
            fill = "steelblue",bins = 5, add_density = TRUE)

stat.test <- result_long1  %>%
  #wilcox_test(prop ~ type, paired = TRUE) %>%
  sign_test(prop ~ type) %>%
  add_significance()
#stat.test
...
effect_size <- result_long1  %>% 
  wilcox_effsize(prop ~ type, paired = TRUE)



## plot ----
fig_diff_group <- ggplot(vigresult, 
...
### model: prop - type + predator +  (year) + (groupID) 
Using social groups as our sample size, we also tested for coordination when predators were (n = 48 scans in 20 observation sessions) and were not (n = 977 scans in 176 observation sessions) observed. For this analysis, we only selected social groups that were observed during at least 2 predator encounters each year (n = 9 social groups). We used a glmmADMB with the proportion of time spent perched as dependent variable, an interaction between proportion type and predator presence as independent variable and year and group ID as random factors. 

model: proportion time spent perching ~ proportion type (observed vs expected) * predator presence + (1|year) + (1|group)

```{r Loop to get coordination/group predators}
glimpse(scanprim8)

# predators 
unique(scanprim8$source)
predators<- c("raven", "shrike", "ravens", "buteo", "cat", "dog", "kestrel", "humans")

scanprim8_pred <- scanprim8 %>% 
  dplyr::mutate(extstimulus =
           ifelse(source==""|source=="unknown"|source== "after pigeon call", "not observed", "predator"))
unique(scanprim8_pred$extstimulus)

total_sum_predators <- scanprim8_pred %>%
  dplyr::group_by( extstimulus) %>%
  dplyr::summarise(n=n_distinct(dop))

sum_predators <- scanprim8_pred %>%
  dplyr::group_by(year, extstimulus, group) %>%
  dplyr::summarise(n=n_distinct(dop)) %>%
  dplyr::ungroup() %>%
  dplyr::filter(group!="", n>1) # only keep groups with over 1 observation session with predator


keep_groups <- sum_predators %>%
  filter(extstimulus=="predator") %>%
  dplyr::select(year, group)

scanprim8_select <- semi_join(scanprim8_pred, keep_groups, by = c("year", "group"))# select scan data of groups that have at least 2 predator encounters to compare with non-predator scans
unique(scanprim8_select$group)
unique(keep_groups$group)

check_groups <- scanprim8_select %>%
  dplyr::group_by(year, group) %>%
  dplyr::summarise(n=n_distinct(dop))

# start loop
unique(scanprim8_select$group)
glimpse(scanprim8_select)

group <- unique(scanprim8_select$group)
...
for(i in seq_along(unique(scanprim8_select$group))){
  run.code <- group[i]
  print(run.code)
  run.data <- subset(scanprim8_select, group==run.code)

# observed proportion at least 1 perched
obs.dur <- dplyr::group_by(run.data, year, extstimulus , id) %>%
          dplyr::summarise( scans.ID = n_distinct(dos))  %>%  # per dop in how many scans was each squirrel observed
  dplyr::summarise(total.scans = sum(scans.ID), 
            grp.size = n_distinct(id)) %>%
  mutate(time.scans= total.scans*600)  # sum duration (scans each ID /dop observed)
...
obs.perch <- run.data %>% 
  dplyr::group_by(year, extstimulus) %>%
  dplyr::filter(behaviour_1 =="perch" | behaviour_2=="perch", duration>30) %>%
  dplyr::summarise(time.perch = sum(duration))  # sum duration perched
#sum(obs.perch$time.perch)

duration <- obs.dur %>% 
  left_join(obs.perch, by = c("year", "extstimulus")) %>% 
  mutate(Pobs= time.perch/time.scans) %>% 
  dplyr::select(year,extstimulus, Pobs)
...
## total time observed
exp.dur <- dplyr::group_by(run.data, year,extstimulus ,  id) %>%
              dplyr::summarise(scans.obs = n_distinct(dos))  %>%  
              dplyr::mutate(time.obs = scans.obs*600)

## total time spent perched
exp.perch <- dplyr::group_by(run.data, year,extstimulus ,  id) %>%
  dplyr::filter(behaviour_1 =="perch" | behaviour_2=="perch", duration>30) %>%
  dplyr::summarise(time.perch = sum(duration))       
...
scanprim8_groupsize_group <- scanprim8 %>% 
  dplyr::group_by(year, group) %>%
  dplyr::summarise(groupsize=n_distinct(id))
result_pred<-left_join(result, scanprim8_groupsize_group , by=c("year", "group"))
...
result.sum <- result_long %>%
  dplyr::group_by(extstimulus, type) %>%
  dplyr::summarize(mean = mean(prop, na.rm = TRUE), 
            sd = sd(prop, na.rm = TRUE),
            n=length(unique(group)),
            se=sd/sqrt(n))

result.sum.year <- result_long %>%
  dplyr::group_by(year, extstimulus, type) %>%
  dplyr::summarize(mean = mean(prop, na.rm = TRUE), 
            sd = sd(prop, na.rm = TRUE),
            n=length(unique(group)),
            se=sd/sqrt(n))
...
# GLMM: prop - type * predator + group size + (1|year) + (1|groupID) or
# GLMM: prop diff - predator + group size + (1|year) + (1|groupID) 
result_long1 <- result_long %>% 
  dplyr::filter(type!="diff") 
glimpse(result_long1)
result_long1$group<- as.factor(result_long1$group)
result_long1$year<- as.factor(result_long1$year)
result_long1$type<-factor(result_long1$type, levels = c("Pobs", "Pexp"))
result_long1$prop[result_long1$prop==0] <- 0.001
  
range(result_long1$prop) 
hist(result_long1$prop)
qqnorm(result_long1$prop)
qqline(result_long1$prop)
shapiro.test(result_long1$prop)
library(fitdistrplus)
descdist(result_long1$prop, discrete = FALSE) 

mfull<- glmmadmb(prop ~ type * extstimulus +  (1|year) + (1|group) ,
                 data = result_long1,
                 family = "beta")
summary(mfull)

#?isSingular
#isSingular(mfull, tol = 1e-4)
...
## plot ----
glimpse(result_long)
vigresults <- result_long %>% filter(P =="diff")

fig_diff_group_ns <- vigresults %>% 
  filter(extstimulus=="not observed") %>%                                               ggplot(aes(x=as.integer(groupsize), y=prop, color=group)) +
                 geom_point() + # shape=1
                 theme_classic() +
                 ggtitle("predator not sighted") +
                 labs(x = "", y= "Pobs - Pexp") +
                 geom_hline(yintercept = 0) +
                 facet_grid(.~year) +
                 scale_color_viridis_d(name="") +
...
fig_diff_group_pred <- vigresults %>% 
  filter(extstimulus=="predator") %>%                                               ggplot(aes(x=as.integer(groupsize), y=prop, color=group)) +
                 geom_point() + # shape=1
                 theme_classic() +
                 ggtitle("predator sighted") +
                 labs(x = "group size", y= "Pobs - Pexp") +
                 geom_hline(yintercept = 0) +
                 facet_grid(.~year) +
                 scale_color_viridis_d(name="") +
...
#ggsave("fig_diff_group_pred.pdf", width = 84, height = 50, units = "mm")
...
ggarrange(fig_diff_group_ns  , fig_diff_group_pred, labels = "auto",
          nrow = 2, ncol=1)
ggsave("fig3.pdf", width = 84, height = 100, units = "mm")
